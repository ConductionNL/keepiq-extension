...
- Website icons are off. Bitwarden loads favicons from its own icon server by default; Keepiq's web app only does so when an admin configured `favicon_service_url`, which the API does not expose, and any third-party icon service leaks the vault's domains. Type icons are used instead. Not forced; recorded for ext-settings to add a "Show website icons" toggle, default off.
