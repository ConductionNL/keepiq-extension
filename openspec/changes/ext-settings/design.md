...
- **The background owns reads and writes.** The popup sends `settings.get` and `settings.set`; the background validates, writes and returns the full snapshot. The popup never touches `storage.local` for settings, matching the existing "background owns state" split. `settings.set` takes a partial patch so one message serves every toggle.
...
- **Keyboard shortcut is read, not owned.** The view calls `browser.commands.getAll()`, which exists on both targets, and shows the first command's shortcut or "Not set" when ext-autofill has not declared one. Chromium allows `browser.tabs.create({ url: 'chrome://extensions/shortcuts' })`; Firefox refuses privileged `about:` URLs from `tabs.create`, so Firefox gets instructions plus a Copy control. Detected with `import.meta.env.FIREFOX`.
...
- **Theme via a root attribute.** The popup sets `data-theme` on `<html>` from the global record before first paint; `popup.css` keeps `color-scheme: light dark` for System default and overrides the tokens under `[data-theme="light"]` and `[data-theme="dark"]`. Compact mode is a second attribute that tightens spacing tokens. Show animations toggles a `prefers-reduced-motion`-style class.
...
- `entrypoints/popup/views/settings/index.ts`: section list and navigation.
- `entrypoints/popup/views/settings/account-security.ts`, `autofill.ts`, `notifications.ts`, `vault.ts`, `appearance.ts`, `about.ts`: one view per section.
...
- The unlock view from ext-accounts-and-unlock (`entrypoints/popup/views/unlock.ts`): PIN field and "Use master password" link when the snapshot reports a PIN.
...
export interface SettingsSnapshot {
	values: SettingsValues            // account and global keys merged, validated, policy applied
